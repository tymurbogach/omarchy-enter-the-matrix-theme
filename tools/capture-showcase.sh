...
# selects the rain as the background and moves to an empty workspace. Keep your
# hands off the keyboard and the mouse while it runs: a key closes the
# screensaver, and a click can move the focus.
#
# On exit, also after a failure or Ctrl-C, it puts back what it found: the
# theme, the background, the workspace and the idle setting.
#
...
ROOT=$(dirname "$HERE")
ALL_SCENES="desktop,screensaver,lock,typing,dialog,exits"
...
for scene in ${SCENES//,/ }; do
  [[ ,$ALL_SCENES, == *",$scene,"* ]] || die "unknown scene $scene"
done
...
# Where the pack links the rain's background (offer_live_background, lib/pack.sh).
LIVE_LINK="$HOME/.config/omarchy/backgrounds/$SLUG/$(basename "$(provider .liveBackground)")"
...
# The rain is the background, and a lock clone of ours is the enabled lock.
pack_up() {
  omarchy-shell "$IPC" status | jq -e .rainIsBackground &&
    omarchy-plugin-list --json |
    jq -e 'any(.[]; .enabled and (.id | endswith(".lock")) and .id != "omarchy.lock")'
}
...
[[ -e $LIVE_LINK ]] || die "the rain's background is not at $LIVE_LINK: run $CLI doctor"
omarchy-theme-bg-set "$LIVE_LINK" >/dev/null
